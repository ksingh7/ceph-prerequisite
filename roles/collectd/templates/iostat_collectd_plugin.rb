...
@output = IO.popen("iostat -Nxm 60")
    while line = @output.gets do
        if  ( line =~ /^(Linux|Time:|avg-cpu|Device| |$)/ )
            #puts "Debug: Skipped line:" + line
        else
	    device, rrqm_sec,  wrqm_sec, r_s, w_s, rMB_s, wMB_s, avgrq_sz, avgqu_sz, await, svctm, _util = line.split
            ...
            puts "PUTVAL " + HOSTNAME.chomp + "/iostatplugin/gauge-" + device + "/rMB " + "N:" + rMB_s
            puts "PUTVAL " + HOSTNAME.chomp + "/iostatplugin/gauge-" + device + "/wMB " + "N:" + wMB_s
            ...
            puts "PUTVAL " + HOSTNAME.chomp + "/iostatplugin/gauge-" + device + "/util " + "N:" + _util
        end
    end
